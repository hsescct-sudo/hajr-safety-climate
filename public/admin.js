window.Admin = (() => {
  ...
  function exportConfig(){download('HAJR_Safety_Climate_V10_3_Config.json',JSON.stringify(config,null,2));}
  function exportJSON(){download('HAJR_Safety_Climate_V10_3_Results.json',JSON.stringify(responses,null,2));}
  function exportCSV(){const rows=[['Timestamp','Campaign','Role','Division','Area','Language','Question ID','Factor','Rating','Normalised rating','Comment']];responses.forEach(r=>(r.answers||[]).forEach(a=>rows.push([r.timestamp,r.campaign||'Legacy / Previous',responseRole(r),r.division,r.area,r.language,a.qid,a.factor,a.value,value(a),a.comment||''])));download('HAJR_Safety_Climate_V10_3_Results.csv',rows.map(row=>row.map(x=>`"${String(x??'').replaceAll('"','""')}"`).join(',')).join('\n'),'text/csv');}
