window.Admin = (() => {
  ...
  async function saveNow(manual=true){
    ...
    try{
      config.version=10;config.schema='hajr-safety-climate-v10';
  ...
  function renderAll(){renderLanguageSelectors();renderRoleSelector();renderDashboardTabs();renderDashboard();renderSettings();renderQuestionBuilder();renderAudience();notifyDashboard();}
  ...
  function renderSettings(){const l=$('contentLang').value||'en';$('projectCode').value=config.project.code||'';$('projectName').value=T(config.project.name,l);$('surveyNameEdit').value=T(config.project.surveyName,l);$('surveySubtitleEdit').value=T(config.project.subtitle,l);if($('campaignName'))$('campaignName').value=config.campaign?.name||'Baseline Safety Climate Survey 2026';if($('campaignStatus'))$('campaignStatus').value=config.campaign?.status||'Open';$('primaryColor').value=config.theme?.primary||'#071a3a';$('accentColor').value=config.theme?.accent||'#087a4a';$('logoEditors').innerHTML=(config.logos||[]).map((x,i)=>`<div class="logo-editor"><img src="${E(x.src)}"><div style="font-weight:850;margin:7px 0">${E(x.name)}</div><label class="small"><input type="checkbox" ${x.enabled!==false?'checked':''} onchange="Admin.toggleLogo(${i},this.checked)"> Enabled</label><div class="inline-actions" style="justify-content:center;margin-top:7px"><label class="btn btn-secondary" style="font-size:10px;padding:6px 8px">Replace<input type="file" accept="image/*" hidden onchange="Admin.replaceLogo(${i},this.files[0]);this.value=''"></label><button class="btn btn-danger" style="font-size:10px;padding:6px 8px" onclick="Admin.deleteLogo(${i})">Delete</button></div></div>`).join('');}
  ...
  function exportConfig(){download('HAJR_Safety_Climate_V10_2_Config.json',JSON.stringify(config,null,2));}
  function exportJSON(){download('HAJR_Safety_Climate_V10_2_Results.json',JSON.stringify(responses,null,2));}
  function exportCSV(){const rows=[['Timestamp','Campaign','Role','Division','Area','Language','Question ID','Factor','Rating','Normalised rating','Comment']];responses.forEach(r=>(r.answers||[]).forEach(a=>rows.push([r.timestamp,r.campaign||'Legacy / Previous',responseRole(r),r.division,r.area,r.language,a.qid,a.factor,a.value,value(a),a.comment||''])));download('HAJR_Safety_Climate_V10_2_Results.csv',rows.map(row=>row.map(x=>`"${String(x??'').replaceAll('"','""')}"`).join(',')).join('\n'),'text/csv');}
